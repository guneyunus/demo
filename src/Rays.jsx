...
import { SparklyMaterial } from "./ShinyMaterial";
...
const sph2 = new THREE.SphereGeometry(0.25, 5, 5);
const bas = new THREE.MeshBasicMaterial();
const tra = new THREE.MeshBasicMaterial({
  transparent: true,
  opacity: 0.25,
  color: "red",
});
...
export const Rays = (props) => {
  const ref = useRef();
  const { count } = useControls({ count: { value: 10, min: 0, max: 500 } });
  return (
    <>
      <group ref={ref} {...props} />
...
const Ray = ({ target }) => {
  ...
  return (
    <group ref={objRef}>
      <mesh ref={origMesh} geometry={sph}>
        <SparklyMaterial color="red" speed={1.5} />
      </mesh>
      <mesh ref={hitMesh} geometry={sph2}>
        <SparklyMaterial color="orange" speed={1.5} />
      </mesh>
